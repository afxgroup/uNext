...
void PauseMenu::Draw(SDL_Renderer* rR) {
	...
	int X = CCFG::GAME_WIDTH / 2 - CCFG::getText()->getTextWidth(lMO[0]->getText(), 16) / 2;
	for(unsigned int i = 0; i < lMO.size(); i++) {
		...
		int newX = CCFG::GAME_WIDTH / 2 - CCFG::getText()->getTextWidth(lMO[i]->getText(), 16) / 2;
		if (newX < X)
			X = newX;
	}
	CCFG::getMM()->getActiveOption()->Draw(rR, X - 32, lMO[activeMenuOption]->getYPos());
