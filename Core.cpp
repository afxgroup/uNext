...
CCore::CCore(void)
{
	...
	auto keyLeft   = op.add<popl::Value<int>>("a", "keyLeft", "Left key", SDLK_a);
	auto keyDown   = op.add<popl::Value<int>>("s", "keyDown", "Down key", SDLK_s);
	auto keyRight  = op.add<popl::Value<int>>("d", "keyRight", "Right key", SDLK_d);
	auto keySpace  = op.add<popl::Value<int>>("j", "keySpace", "Space key", SDLK_SPACE);
	auto keyRun    = op.add<popl::Value<int>>("r", "keyRun", "Run key", SDLK_LSHIFT);
	auto volume    = op.add<popl::Value<int>>("v", "volume", "Volume Level", 100);
	auto canMoveBw = op.add<popl::Value<bool>>("w", "canMoveBw", "Can Move Backward", true);
	auto fullscreen = op.add<popl::Value<bool>>("f", "fullscreen", "Fullscreen", false);

	std::ifstream file("settings.ini");
	if (!file) {
		CCFG::createDefaultSettings();
	}
	else
		file.close();

	op.parse("settings.ini");
	if (volume->value() < 0)
		volume->value(0);
	if (volume->value() > 100)
		volume->value(100);
	...
	window = SDL_CreateWindow("uNext", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, CCFG::GAME_WIDTH, CCFG::GAME_HEIGHT, SDL_WINDOW_SHOWN);
	...
	CCFG::fullscreen = fullscreen->value();
	if (fullscreen->value())
	...
	CCFG::canMoveBackward = canMoveBw->value();	
	CCFG::setVolume(volume->value());
	...
	CCFG::initKeys(keyLeft->value(), keyDown->value(), keyRight->value(), keySpace->value(), keyRun->value());
	CCFG::saveSettings();
}
...
void CCore::mainLoop()
{
	...
	while (!quitGame && mainEvent->type != SDL_QUIT)
	{
		...
		bool currentFullscreen = windowFlags & SDL_WINDOW_FULLSCREEN;
		if (CCFG::fullscreen != currentFullscreen) {
			SDL_SetWindowFullscreen(window, CCFG::fullscreen);
		}
